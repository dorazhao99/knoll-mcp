...
interface ModulesResponse {
  modules: string[] | number[] | any[]; 
  checked: boolean[] | string[] | any[];
}

async function makeKnowledgeRequest<T>(): Promise<T | null> {
  ...
  let moduleInfo = await axios.get<ModulesResponse>(modulesUrl, { params: modulesHeader })
  if (moduleInfo.data) {
    knowledgeHeaders.modules = moduleInfo.data.modules
    knowledgeHeaders.checked = moduleInfo.data.checked
  }

  let knowledgeInfo = await axios.post<KnowledgeResponse>(knowledgeUrl, knowledgeHeaders)
  if (knowledgeInfo.data) {
    const updatedKnowledge = JSON.stringify(knowledgeInfo.data) 
    return {success: true, response: updatedKnowledge} as T
  } else {
...
server.tool(
  ...
  "Insert activated knowledge modules",
  {},
  async () => {
    console.error("Using tool")
    const knowledgeData = await makeKnowledgeRequest<KnowledgeResponse>();
    ...
    if (knowledgeData.response) {
      ...
      Object.keys(knowledgeModules).forEach((v: string) => {
